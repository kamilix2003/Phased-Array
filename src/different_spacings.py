...
def main():    
  ...
  for i, d in enumerate(ds):
    ...
    for b in range(beta.shape[0]):
      ...
      ax.plot((theta_deg), ep_db, color='black', linewidth=.5, linestyle='--')
      ax.plot((theta_deg), ap_db[b, :], label=f'd={((d[1]) / (c/f)):.2f} lambda, beam direction={main_lobe_direction(ap_db[b, :], theta, format='degrees'):.1f} degrees', linestyle=line_styles[i], color=line_colors[b])
